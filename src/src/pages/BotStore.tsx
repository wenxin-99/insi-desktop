    ...
    <Card className="group relative overflow-hidden hover:shadow-md transition-all duration-200 hover:border-primary/30">
      <div className="p-4">
        ...
        <div className="flex gap-2">
          ...
          {!isOwner && onLike && (
            <Button size="sm" variant="outline" className={`h-8 w-8 p-0 ${isLiked ? 'text-red-500 border-red-200 bg-red-50 hover:bg-red-100' : ''}`} onClick={onLike} title={isLiked ? '取消点赞' : '点赞'}>
...
export default function BotStore() {
  ...
  const { data: myLikes, refetch: refetchMyLikes } = trpc.customBot.myLikes.useQuery();
  ...
  const likeMut = trpc.customBot.toggleLikeV2.useMutation({
    onSuccess: () => { refetchStore(); refetchMyLikes(); },
  });
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {featuredBots.map((bot: any) => (
                <BotCard key={bot.id} bot={bot} isOwner={false} onUse={() => handleUse(bot.id)}
                  onLike={() => likeMut.mutate({ botId: bot.id })} isLiked={likedSet.has(bot.id)} />
              ))}
            </div>
          </div>
        )}
