...
export function ChatInputArea({
  ...
  function handleSend() {
    ...
    // ★ 如果选择了知识库，在消息前附加 @知识库名 引用
    const kbPrefix = selectedKB ? `@${selectedKB.name} ` : '';
    const prefix = ghPrefix + kbPrefix;

    // 发送后清空知识库选择
    const clearKB = () => { if (selectedKB) setSelectedKB(null); };
    ...
    if (quotedRef) {
      if (quotedRef.type === 'image' && quotedRef.imageUrls?.length) {
        ...
        setQuotedRef(null);
        clearKB();
        setTimeout(() => handleSendMessage(prefix + (text || '')), 50);
        return;
      }
      if (quotedRef.type === 'video' && quotedRef.videoTaskId) {
        ...
        setQuotedRef(null);
        clearKB();
        handleSendMessage(prefix + videoContext + (text || ''));
        return;
      }
      ...
      if (quotedRef.type === 'message' && quotedRef.messageContent) {
        ...
        setQuotedRef(null);
        clearKB();
        handleSendMessage(prefix + msgContext + (text || ''));
        return;
      }
      setQuotedRef(null);
    }
                ...
                <div className="flex-1 min-w-0 relative">
                  ...
                  <ChatInput
                    ...
                    onSend={(payload: SendMessagePayload) => {
                      ...
                      const kbTag = selectedKB ? `@${selectedKB.name} ` : '';
                      if (selectedKB) setSelectedKB(null);
                      handleSendMessage(kbTag + payload.text);
                    }}
                    onPaste={async (e) => {
                ...
                <PressToTalkButton
                  onTranscribed={(text) => {
                    if (voiceSettings.mode === 'auto-send') {
                      ...
                      const kbTag = selectedKB ? `@${selectedKB.name} ` : '';
                      if (selectedKB) setSelectedKB(null);
                      setTimeout(() => handleSendMessage(kbTag + text), 50);
                    } else {
